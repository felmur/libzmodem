...
const char *program_name2 = "sz";


size_t zmodem_send(int file_count,
		   const char **file_list,
		   bool (*tick)(const char * zz, long bytes_sent, long bytes_total, long last_bps, int min_left, int sec_left),
		   ...
		   uint32_t flags)
{
	...
	if (sz_transmit_files(sz, file_count, (char **)file_list)==ERROR) {
		sz->exitcode=0200;
...
sz_transmit_files (sz_t *sz, int argc, char *argp[])
{
	...
	for (n = 0; n < argc; ++n) {
		...
		if (sz_transmit_file (sz, (const char *)argp[n], NULL) == ERROR)
			return ERROR;
	}
	sz->totsecs = 0;
