...
tcp_server (char *buf)
{
	...
	len=sizeof(t);
	if (getsockname (sock, (struct sockaddr *) &t, (socklen_t *restrict) &len)) {
	...
	getsockname (sock, (struct sockaddr *) &t, (socklen_t *restrict) &len);
...
retry:
	...
	alarm(30);
	if ((so = accept(d, (struct sockaddr*)&s, (socklen_t *restrict) &namelen)) < 0) {
		if (errno == EINTR) {
			if (++num<=5)
				goto retry;
		}
